...
/**
 * Locates the {@link org.eclipse.jdt.core.dom.BodyDeclaration} associated with an enum declaration.
 */
public final class EnumConstantLocater implements BodyDeclarationLocater {
    ...
    public BodyDeclaration find(CompilationUnit cu) {
        ...
        for (EnumConstantDeclaration enumConstantDeclaration : (List<EnumConstantDeclaration>) ((EnumDeclaration) typeDeclaration).enumConstants()) {
            if (enumConstantDeclaration.getName().getFullyQualifiedName().equals(constantName)) {
