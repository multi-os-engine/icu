...
public class DecimalFormat extends NumberFormat {
    ...
    private final boolean subparse(
        ...
        if (text.regionMatches(position, symbols.getInfinity(), 0,
            ...
            UnicodeSet groupEquiv = skipExtendedSeparatorParsing ? UnicodeSet.EMPTY :
                getEquivalentGroupings(grouping, strictParse, decimalEquiv);
    ...
    // Google patch
    /** package private for access by DecimalFormatSymbols */
    static char getLatinSeparator(char separator) {
        if (separator < 0x0100) {
            return separator;
        }

        if (dotEquivalents.contains(separator)) {
            return '.';
        }
        if (commaEquivalents.contains(separator)) {
            return ',';
        }
        return '\u00a0';
    }

    // Google Patch: Public ICU is very lenient even in strict mode, almost any
    // dot or comma is a grouping separator. Parsing strings like "1.234" in French
    // was treating '.' like an ignorable grouping separator, and we want it to be
    // excluded.  We keep the public behavior when strictParse is false, but when
    // it is true we restrict grouping separators to the smaller set of equivalents.
    private UnicodeSet getEquivalentGroupings(char grouping, boolean strictParse,
                                              UnicodeSet decimalEquiv) {

        UnicodeSet equivSet = getEquivalentDecimals(grouping, strictParse);
        if (equivSet == decimalEquiv) {
            // oops, data error, ignore this
            return UnicodeSet.EMPTY;
        }
        if (equivSet == UnicodeSet.EMPTY) {
            // note, not defaultGroupingSeparators, which contains commas and decimals
            if (strictOtherGroupingSeparators.contains(grouping)) {
                equivSet = strictOtherGroupingSeparators;
            }
        }
        return equivSet;
    }
